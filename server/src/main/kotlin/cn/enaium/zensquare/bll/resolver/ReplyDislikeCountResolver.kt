 ...
 */
@Component
class ReplyDislikeCountResolver(val sql: KSqlClient) : KTransientResolver<UUID, Long> {

    /**
     * count dislike by ids
     *
     * @param ids ids
     * @return count
     */
    override fun resolve(ids: Collection<UUID>): Map<UUID, Long> = sql.createQuery(Reply::class) {
        where(table.id valueIn ids, table.asTableEx().likes.dislike eq true)
        groupBy(table.id)
